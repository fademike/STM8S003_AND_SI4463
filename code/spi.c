...
int spi_cs_init(void){
	PORT(PD, DDR)  |= GPIO_PIN2;
	PORT(PD, CR1)  |= GPIO_PIN2;
	return 0;
}

void spi_cs(int pos){
	if (pos != 0)PORT(PD, ODR) |= GPIO_PIN2;
	else PORT(PD, ODR) &= ~GPIO_PIN2;
}

int spi_ce_init(void){
	PORT(PD, DDR)  |= GPIO_PIN4;
	PORT(PD, CR1)  |= GPIO_PIN4;
	return 0;
}

#define CE_INVERCE 1
void spi_ce(int pos){
	if (pos != 0)PORT(PD, ODR) |= GPIO_PIN4;
	else PORT(PD, ODR) &= ~GPIO_PIN4;
}

int spi_txrx(U8 * buf_tx, U8* buf_rx, int num){
	int i=0;
	for (i=0;i<num;i++)buf_rx[i] = spi_send_byte(buf_tx[i]);
    return num;
}
...
void spi_init(){
	...
	SPI_CR1 = SPI_CR1_SPE | // spi enable
			(0 << 3) | // //bitrate	div: 0 - mfreq/2, 256 - mfreq/256
